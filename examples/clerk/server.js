...
app.use(tideless({ resolveUid: async (req) => (await clerk.currentUser(req).catch(() => null))?.id, role: "vault-reader" }));
// Link a Tide identity to the signed-in user (see ../shared/link.js). It pins the sign-in to its own
// cookie, which is exactly what a Clerk dev instance needs: it cannot identify the caller on the
// navigation back from the enclave.
app.use(link({
  resolveUser: (req) => clerk.currentUser(req).catch(() => null),
  storeVuid: (id, vuid) => clerk.storeVuid(id, vuid),
  appUrl: APP_URL,
  afterLink: "/",
}));
