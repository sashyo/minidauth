...
signed-in Supabase user encrypts and decrypts with **no Tide account and no doken**. Their own
the Supabase user id is the subject, and a role a quorum granted that id is the gate; minidauth issues the
...
- Grant the user's id a role as a tideless subject, through the quorum:
  ...
  curl -sX POST localhost:8081/iga/change-requests/role -H "Authorization: Bearer $ALICE" \
    -H 'Content-Type: application/json' -d '{"vuid":"<supabase user id>","role":"vault-reader","tideless":true}'
